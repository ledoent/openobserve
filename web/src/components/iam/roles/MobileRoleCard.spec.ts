...
const mountCard = (row: Record<string, any>) =>
  mount(MobileRoleCard, {
    props: { row },
    global: { plugins: [Quasar] },
  });

describe("MobileRoleCard", () => {
  const baseRow = { role_name: "admin" };

  it("renders the role name", () => {
    const w = mountCard(baseRow);
    expect(w.find(".mobile-role-card__title").text()).toBe("admin");
  });

  it("emits click with row on tap", async () => {
    const w = mountCard(baseRow);
    await w.find(".mobile-role-card").trigger("click");
  ...
  it("emits click on Enter keydown", async () => {
    const w = mountCard(baseRow);
    await w.find(".mobile-role-card").trigger("keydown.enter");
    expect(w.emitted("click")).toBeTruthy();
  });
  ...
  it("does not propagate card click from the overflow button", async () => {
    const w = mountCard(baseRow);
    const more = w.find(".mobile-role-card__more");
